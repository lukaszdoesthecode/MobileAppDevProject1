    ...
    private fun registerUser() {
        ...
        lifecycleScope.launch {
            try {
                ...
                val hashedPassword = Bcrypt.hash(userPassword, 12)
                val hashedPasswordString = Base64.encodeToString(hashedPassword, Base64.DEFAULT)

                val userId = SuspendedQueriesUser.getUserId(userUsername)

                val authData = AuthData(userId, userEmail, hashedPasswordString, Role.USER.toString())
                ...
                Log.e("User", "Registration process failed", e)
